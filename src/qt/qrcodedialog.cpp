...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("FIC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("fincoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FIC unit input in lnReqAmount, we generate the URI with FIC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::FIC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
